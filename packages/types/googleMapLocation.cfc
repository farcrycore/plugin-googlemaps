<cfcomponent extends="farcry.core.packages.types.types" displayName="Google Map Location" hint="Locations for use as markers on Google Maps" bFriendly="1" bobjectbroker="true" objectbrokermaxobjects="1000">
...
--->
	...
	<cfproperty ftSeq="31" ftFieldSet="Geocoding" name="Geocode" type="longchar" hint="An actual address to use geocoding to render the map" required="false" ftLabel="Physical address" ftType="geoAddress" />
...
<!--- Get Image details for a library image --->
	<cffunction name="getIconDetail" access="public" displayname="Get Icon Details for Map Location" hint="returns a structure containing url, height and width for an associated icon image" returntype="struct" output="false">
		...
		<cfset stIconDetail = oImageUtils.fGetProperties(expandPath(oImage.getURLImagePath(arguments.ObjectID,"original"))) />

		<cfset stIconDetail.IconURL = oImage.getURLImagePath(arguments.ObjectID,"original") />
<!--- 		<cfset stIconDetail.height = 32 />
		<cfset stIconDetail.width = 32 /> --->
